...
const io = socketIo(server, {
  cors: {
    origin: ["http://localhost:5173", "https://code-clash-front.vercel.app/"],
    methods: ["GET", "POST"],
  },
});
...
app.get("/", (req, res) => {
  res.send("Code Clash Arena backend is running!");
});
// Routes
...
io.of("/arena").on("connection", (socket) => {
  ...
  socket.on("join_queue", async (data) => {
    ...
    if (!waitingPlayer) {
      ...
      console.log(
        `Creating match between ${player1.userId} and ${player2.userId}`
      );
  ...
  socket.on("player_finished", async (data) => {
    ...
    console.log(
      `💻 Winner's code preview:`,
      code ? code.substring(0, 100) + "..." : "No code"
    );
    ...
    try {
      ...
      if (arena) {
        ...
        if (user) {
          ...
          await user.save();
          console.log(
            `🪙 Awarded ${arena.tokenPrize} tokens to ${userId}. New total: ${user.tokens}`
          );
          ...
          console.log(
            `💾 Stored winner info: ${user.username}, ${user.tokens} tokens`
          );
        }
        ...
        console.log(
          `✅ Winner events sent: ${arena.tokenPrize} tokens awarded`
        );
